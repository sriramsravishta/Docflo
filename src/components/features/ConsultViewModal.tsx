...
function MedicationsTable({
  ...
  // ✅ Column-level: if ANY row has content, cap column at 300px (else 120px)
  ...
  // ✅ IMPORTANT: use explicit w + max-w to enforce cap in tables
  const COL_INSTR = `min-w-[120px] ${
    hasAnyInstructions ? 'w-[300px] max-w-[300px]' : 'w-[120px] max-w-[120px]'
  }`;
  const COL_FLAGS = `min-w-[120px] ${
    hasAnyFlags ? 'w-[300px] max-w-[300px]' : 'w-[120px] max-w-[120px]'
  }`;
  ...
  // ✅ helper class for the inner wrapper so table layout respects the cap
  const instrWrapClass = hasAnyInstructions
    ? 'w-[300px] max-w-[300px] overflow-hidden'
    : 'w-[120px] max-w-[120px] overflow-hidden';

  const flagsWrapClass = hasAnyFlags
    ? 'w-[300px] max-w-[300px] overflow-hidden'
    : 'w-[120px] max-w-[120px] overflow-hidden';
          ...
          {consultMedicines.map((m) => {
            ...
            return (
              <tr key={m.id} className="hover:bg-gray-50">
                ...
                {/* ✅ Instructions: capped at 300px if any content exists in column */}
                ...
                {/* ✅ Flags: capped at 300px if any content exists in column */}
