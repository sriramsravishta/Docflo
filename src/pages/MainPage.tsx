        ...
        <div className="space-y-20">
         <section>
           ...
           {activeTab === 'today' && (
            ...
            {activeTab === 'leads' && hasLeadsFeature && (
              <div className="space-y-6">
                <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
                  {['All', 'Dr. Sushma Peruri', 'Dr. Prashanth Koyyoda'].map((doc) => (
                    <button
                      key={doc}
                      onClick={() => setLeadFilter(doc as any)}
                ...
                {leadsLoading ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {leads
                      .filter(l => leadFilter === 'All' || l.doctor_label === leadFilter)
                      .map((lead) => (
                        <div key={lead.id} className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm flex flex-col gap-4 hover:shadow-md transition-shadow">
                          <div>
                            <div className="flex justify-between items-start mb-2">
                              <h3 className="font-semibold text-gray-900 text-lg">{lead.lead_name}</h3>
                              <span className="inline-flex items-center text-xs font-medium bg-gray-100 text-gray-600 px-2 py-1 rounded">
                                {lead.source || 'Website'}
                              </span>
                            </div>
                            ...
                            <div className="space-y-2 text-sm text-gray-600">
                              <div className="flex items-center gap-2">
                                ...
                                <a href={`tel:${lead.phone}`} className="hover:text-[#024CDB] font-medium">{lead.phone}</a>
                                <span className="text-gray-300 mx-1">|</span>
                                <a href={`https://wa.me/91${lead.phone.replace(/\D/g, '').slice(-10)}`} target="_blank" rel="noreferrer" className="text-green-600 font-medium hover:underline">WhatsApp</a>
                              </div>
                              {lead.service && (
                                <div className="flex items-center gap-2">
                                  <User className="w-4 h-4 text-gray-400 shrink-0" />
                                  <span className="truncate" title={lead.service}>{lead.service}</span>
                                </div>
                              )}
                          ...
                          <div className="pt-3 border-t border-gray-100">
                            ...
                            <select 
                              ...
                              className={`w-full text-sm border-gray-300 rounded-md shadow-sm focus:border-[#024CDB] focus:ring-[#024CDB] ${
                                lead.status === 'new' ? 'bg-blue-50 text-blue-700 font-medium' :
                                lead.status === 'contacted' ? 'bg-yellow-50 text-yellow-700 font-medium' :
                                lead.status === 'booked' ? 'bg-green-50 text-green-700 font-medium' :
                            ...
                            <textarea
                              ...
                              onBlur={(e) => {
                                if (e.target.value !== lead.notes) {
                                  handleLeadNoteSave(lead.id, e.target.value);
                                }
                              }}
                              placeholder="Add follow-up notes..."
                              className="w-full text-sm border-gray-300 rounded-md shadow-sm focus:border-[#024CDB] focus:ring-[#024CDB] min-h-[60px] resize-y"
                            />
                          </div>

                          <button 
                            onClick={() => handleCreateFromLead(lead)}
                            className="w-full mt-auto py-2 px-4 border border-[#024CDB] text-[#024CDB] rounded-lg text-sm font-medium hover:bg-blue-50 transition-colors"
                          >
                            Create Appointment
                          </button>
                        </div>
                    ))}
                    {leads.filter(l => leadFilter === 'All' || l.doctor_label === leadFilter).length === 0 && (
