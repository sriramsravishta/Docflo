  ...
  const handleDocumentUploadOkay = () => {
    ...
    if (printHeaderEnabled) {
      ...
      const hasLogo = !!logoUrl;
        ...
        <div class="header-info"${hasLogo ? ' style="text-align:right"' : ''}>
...
@page{margin-top:${printHeaderEnabled ? '12mm' : '160px'};margin-bottom:${printFooterEnabled ? '12mm' : '130px'};margin-left:12mm;margin-right:12mm}
...
@media print{body{margin:0}}
    ...
    const [appt, doctorNameResult, chartData, presConfigResult] = await Promise.all([
      ...
      cachedDoctorNameRef.current
        ...
        : user?.id
          ? supabase.from('organizations').select('name').eq('auth_id', user.id).single()
              .then(({ data }) => { const name = data?.name || user?.user_metadata?.display_name || user?.user_metadata?.name || '—'; cachedDoctorNameRef.current = name; return name; })
              .catch(() => user?.user_metadata?.display_name || user?.user_metadata?.name || '—')
          : Promise.resolve(user?.user_metadata?.display_name || user?.user_metadata?.name || '—'),
      attachedChartNames.length > 0 && user?.id
        ? supabase.from('diet_charts').select('name, file_urls').eq('doc_id', user.id).in('name', attachedChartNames)
      ...
      user?.id
        ? supabase.from('organizations').select('prescription_config').eq('auth_id', user.id).single()
            .then(({ data }) => data?.prescription_config || {}).catch(() => ({}))
        : Promise.resolve({}),
    ]);
    const referredBy = appt?.referred_by || undefined;
    ...
    styleEl.id = 'rx-dl-styles';
    styleEl.textContent = `#rx-dl *{box-sizing:border-box}#rx-dl{font-family:Arial,sans-serif;line-height:1.6;color:#111;font-size:14px;background:#fff}#rx-dl .pres-wrapper{border:1.5px solid #111;margin:0;padding:0}#rx-dl .pt-info{padding:14px 16px 12px 16px;border-bottom:1px solid #ccc}#rx-dl .pt-row{display:grid;grid-template-columns:1fr 1fr;gap:4px 32px;margin-bottom:2px}#rx-dl .pt-name{font-size:16px;font-weight:700;text-transform:uppercase;color:#111;margin:0 0 4px 0}#rx-dl .pt-meta{font-size:13px;color:#333}#rx-dl .pt-label{font-weight:400;color:#555}#rx-dl .pt-val{font-weight:400;color:#111}#rx-dl .pt-date-val{font-weight:700;color:#111}#rx-dl .section{margin:0;padding:12px 16px;border-bottom:1px solid #ccc}#rx-dl .section:last-child{border-bottom:none}#rx-dl .section-header{font-size:14px;font-weight:700;color:#111;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.02em}#rx-dl .section-text{margin:4px 0;font-size:14px;color:#222}#rx-dl .sub-label{font-size:12px;font-weight:700;color:#444;margin:8px 0 4px 0;text-transform:uppercase;letter-spacing:0.03em}#rx-dl .section-list{margin:4px 0 4px 18px;padding:0}#rx-dl .section-list li{font-size:14px;margin-bottom:3px;color:#222}#rx-dl .inv-priority{font-size:12px;color:#666;font-style:italic}#rx-dl .med-table{width:100%;border-collapse:collapse;margin-top:6px;font-size:13px}#rx-dl .med-table thead tr{background:#f3f4f6}#rx-dl .med-table th{text-align:left;padding:7px 8px;font-size:12px;font-weight:700;border:1px solid #d1d5db;color:#333}#rx-dl .med-table td{padding:7px 8px;border:1px solid #d1d5db;vertical-align:top;color:#222}#rx-dl .th-num,#rx-dl .td-num{width:28px;text-align:center}#rx-dl .th-man,#rx-dl .td-man{width:90px;text-align:center}#rx-dl .th-dur,#rx-dl .td-dur{width:80px}#rx-dl .th-detail,#rx-dl .td-detail{width:140px}#rx-dl .td-name strong{font-size:13px;font-weight:700}#rx-dl .med-sub{font-size:12px;color:#555;margin-top:2px}#rx-dl .med-instruction{font-size:12px;color:#555;margin-top:3px;font-style:italic}#rx-dl .row-even{background:#fff}#rx-dl .row-odd{background:#f9fafb}#rx-dl .man-grid{border-collapse:collapse;margin:0 auto;font-size:11px}#rx-dl .man-val{font-weight:700;text-align:center;padding:1px 4px;color:#111}#rx-dl .man-label{font-size:10px;text-align:center;color:#555;padding:0 4px}#rx-dl .man-sep{text-align:center;padding:1px 1px;color:#999;font-weight:400}#rx-dl .man-legend{font-size:10px;color:#666;margin-top:6px;font-style:italic}#rx-dl .signature-wrapper{break-inside:avoid}#rx-dl .signature{text-align:right;padding:24px 20px 16px 16px}#rx-dl .sig-name{font-size:14px;font-weight:700;text-transform:uppercase;margin:0 0 2px 0;color:#111}#rx-dl .sig-dept{font-size:13px;font-weight:400;color:#111;margin:0 0 2px 0}#rx-dl .sig-date{font-size:12px;color:#555;margin:0}#rx-dl .pres-header{display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:2px solid #e5e7eb;gap:16px}#rx-dl .header-info{flex:1;text-align:left}#rx-dl .header-logo{flex-shrink:0}#rx-dl .header-logo img{height:48px;width:auto;object-fit:contain;max-width:160px}#rx-dl .header-name{font-size:17px;font-weight:700;color:#111;margin:0;letter-spacing:0.01em}#rx-dl .header-qual{font-size:11px;color:#555;margin:2px 0 0;font-weight:400}#rx-dl .header-contact{font-size:11px;color:#555;margin:2px 0 0}#rx-dl .pres-footer{border-top:1px solid #d1d5db;padding:8px 20px;text-align:center}#rx-dl .pres-footer p{font-size:10.5px;color:#555;margin:1px 0}`;
    document.head.appendChild(styleEl);
    const container = document.createElement('div');
    container.id = 'rx-dl';
    container.style.cssText = 'position:absolute;left:0;top:0;width:794px;z-index:-1;pointer-events:none;';
              ...
              <div className="flex flex-col items-end shrink-0">
                {isConsultProcessed(consult) ? (
                  ...
                  <div className="flex items-center gap-3">
                    {(consult as any).summary_streaming ? (
                      <>
                        <div className="relative w-9 h-9">
                          <svg className="w-9 h-9 -rotate-90" viewBox="0 0 36 36">
                            <path className="text-gray-200" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="3" />
                            <path className="text-[#024CDB]" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="3" strokeDasharray={`${Math.round(((consult as any).summary_streaming?.completed_sections?.length || 0) / 12 * 100)}, 100`} strokeLinecap="round" />
                          </svg>
        ...
        <div className="text-sm text-gray-900 truncate">
          ...
          <div className="p-6 space-y-8">
            ...
            <section>{renderHistoryTab()}</section>
                    ...
                    {/* ─── INPATIENT TAB ─── */}
          {SHOW_INPATIENT_TAB && activeProfileTab === 'inpatient' && (
            <InpatientTab
              patientId={patientId!}
              userId={user?.id}
              patient={patient}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Gender</label>
                            <select value={editForm.gender} onChange={(e) => setEditForm({ ...editForm, gender: e.target.value })} className="input-field bg-white">
